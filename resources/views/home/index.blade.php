...
@section('title', 'Gantt chart task')
@section('content')
    ...
    <script type="text/javascript">
        ...
        function getDemoProject(){
            var taskGantt = {!! $taskGanttJson !!};
            var resources = {!! $dataResources !!}

            ret= {"tasks":    taskGantt, "selectedRow": 2, "deletedTaskIds": [], "zoom": "1M",
                "resources": resources,
                "roles":       [
                    {"id": "tmp_1", "name": "Project Manager"},
                    {"id": "tmp_2", "name": "Leader"},
                    {"id": "tmp_3", "name": "Worker"},
    ...
    <div id="gantEditorTemplates" style="display:none;">
        <div class="__template__" type="GANTBUTTONS">
            <div class="ganttButtonBar noprint">
                @if($getProject)
                <div class="project_title">{!! $getProject['name'] !!}</div>
                @endif
              <div class="buttons">
              ...
              <button onclick="editResources();" class="button textual requireWrite" title="edit resources"><span class="teamworkIcon">M</span></button>
                ...
                <div class="search-project">
                    <form action="/">
                        <select class="formElements" name="projectId" onchange="this.form.submit()">
  ...
  <table class="gdfTable" cellspacing="0" cellpadding="0">
    ...
    <tr style="height:40px">
      ...
      <th class="gdfColHeader gdfResizable" style="width:80px;">Duration</th>
      <th class="gdfColHeader gdfResizable" style="width:80px;">% Done</th>
      <th class="gdfColHeader gdfResizable requireCanSeeDep" style="width:80px;">Depends</th>
      <th class="gdfColHeader gdfResizable" style="width:120px;">Status</th>
  ...
  <tr id="tid_(#=obj.id#)" taskId="(#=obj.id#)" class="taskEditRow (#=obj.isParent()?'isParent':''#) (#=obj.collapsed?'collapsed':''#)" level="(#=level#)">
    ...
    <td class="gdfCell"><div class="statusRow taskStatus cvcColorSquare" status="(#=obj.status#)">(#=obj.status_title#)</div></td>
  ...
  --></div>
    ...
    @foreach($status as $item)
        <div class="taskStatus cvcColorSquare" status="{{ $item['alias'] }}" title="{{ $item['title'] }}"></div>
    @endforeach
    </div>
  --></div>
      ...
      <tr>
        <td  colspan="2">
          ...
          <select id="status" name="status" class="taskStatus" status="(#=obj.status#)"  onchange="$(this).attr('STATUS',$(this).val());">
            @foreach($status as $item)
            <option value="{{ $item['alias'] }}" class="taskStatus" status="{{ $item['alias'] }}" >{{ $item['title'] }}</option>
            @endforeach
          </select>
        </td>
